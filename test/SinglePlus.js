...
contract("SinglePlus", async ([owner, strategist, user1, user2, user3]) => {
    ...
    it("should be able to mint", async () => {
        ...
        assert.strictEqual((await plus.getMintAmount("6000000")).toString(), web3.utils.toWei("6"));
        await plus.mint("6000000", {from: user1});

        assert.strictEqual((await plus.index()).toString(), web3.utils.toWei("1"));
        assert.strictEqual((await plus.totalSupply()).toString(), web3.utils.toWei("6"));
        assert.strictEqual((await plus.totalShares()).toString(), web3.utils.toWei("6"));
        assert.strictEqual((await plus.userShare(user1)).toString(), web3.utils.toWei("6"));
        assert.strictEqual((await plus.balanceOf(user1)).toString(), web3.utils.toWei("6"));
        assert.strictEqual((await plus.totalUnderlying()).toString(), web3.utils.toWei("6"));
        ...
        assert.strictEqual((await plus.totalSupply()).toString(), web3.utils.toWei("7.2"));
        assert.strictEqual((await plus.totalShares()).toString(), web3.utils.toWei("6"));
        assert.strictEqual((await plus.userShare(user1)).toString(), web3.utils.toWei("6"));
        assert.strictEqual((await plus.balanceOf(user1)).toString(), web3.utils.toWei("7.2"));
        assert.strictEqual((await plus.userShare(user3)).toString(), web3.utils.toWei("0"));
        assert.strictEqual((await plus.balanceOf(user3)).toString(), web3.utils.toWei("0"));
        assert.strictEqual((await plus.totalUnderlying()).toString(), web3.utils.toWei("7.2"));
        assert.strictEqual((await plus.liquidityRatio()).toString(), web3.utils.toWei("1"));

        await token.mint(user3, "3600000");
        await token.approve(plus.address, "3600000", {from: user3});
        assert.strictEqual((await plus.getMintAmount("3600000")).toString(), web3.utils.toWei("3.6"));
        await plus.mint("3600000", {from: user3});

        assert.strictEqual((await plus.index()).toString(), web3.utils.toWei("1.2"));
        assert.strictEqual((await plus.totalSupply()).toString(), web3.utils.toWei("10.8"));
        assert.strictEqual((await plus.totalShares()).toString(), web3.utils.toWei("9"));
        assert.strictEqual((await plus.userShare(user1)).toString(), web3.utils.toWei("6"));
        assert.strictEqual((await plus.balanceOf(user1)).toString(), web3.utils.toWei("7.2"));
        assert.strictEqual((await plus.userShare(user3)).toString(), web3.utils.toWei("3"));
        assert.strictEqual((await plus.balanceOf(user3)).toString(), web3.utils.toWei("3.6"));
        assert.strictEqual((await plus.totalUnderlying()).toString(), web3.utils.toWei("10.8"));
        assert.strictEqual((await plus.liquidityRatio()).toString(), web3.utils.toWei("1"));
    ...
    it("should be able to redeem", async () => {
        ...
        const redeemAmount = await plus.getRedeemAmount(web3.utils.toWei("2.4"));   // plus token has 18 decimals
        assert.strictEqual(redeemAmount[0].toString(), "2376000"); // redeem amount has 6 decimals
        assert.strictEqual(redeemAmount[1].toString(), web3.utils.toWei("0.024000"));    // fee is in 18 decimals

        await plus.redeem(web3.utils.toWei("2.4"), {from: user1});

        assert.strictEqual((await plus.index()).toString(), web3.utils.toWei("1.2"));
        assert.strictEqual((await plus.totalSupply()).toString(), web3.utils.toWei("8.4"));
        assert.strictEqual((await plus.totalShares()).toString(), web3.utils.toWei("7"));
        assert.strictEqual((await plus.userShare(user1)).toString(), web3.utils.toWei("4"));
        assert.strictEqual((await plus.balanceOf(user1)).toString(), web3.utils.toWei("4.8"));
        assert.strictEqual((await plus.userShare(user3)).toString(), web3.utils.toWei("3"));
        assert.strictEqual((await plus.balanceOf(user3)).toString(), web3.utils.toWei("3.6"));
        assert.strictEqual((await plus.totalUnderlying()).toString(), web3.utils.toWei("8.424"));   // 0.024 fee remains!
    ...
    it("should be able to redeem all", async () => {
        ...
        assert.strictEqual((await plus.index()).toString(), web3.utils.toWei("1.2"));
        assert.strictEqual((await plus.totalSupply()).toString(), web3.utils.toWei("3.6"));
        assert.strictEqual((await plus.totalShares()).toString(), web3.utils.toWei("3"));
        assert.strictEqual((await plus.userShare(user1)).toString(), web3.utils.toWei("0"));
        assert.strictEqual((await plus.balanceOf(user1)).toString(), web3.utils.toWei("0"));
        assert.strictEqual((await plus.userShare(user3)).toString(), web3.utils.toWei("3"));
        assert.strictEqual((await plus.balanceOf(user3)).toString(), web3.utils.toWei("3.6"));
        assert.strictEqual((await plus.totalUnderlying()).toString(), web3.utils.toWei("3.672"));   // 0.072 fee remains!
        assert.strictEqual((await plus.liquidityRatio()).toString(), web3.utils.toWei("1.02"));    // 3.672 / 3.6 = 1.02
        ...
        assert.strictEqual((await plus.index()).toString(), web3.utils.toWei("1.224")); // 8.424 * 10**18 / 7
        assert.strictEqual((await plus.totalSupply()).toString(), web3.utils.toWei("3.672"));   // ~ 8.424
        assert.strictEqual((await plus.totalShares()).toString(), web3.utils.toWei("3"));
        assert.strictEqual((await plus.userShare(user1)).toString(), web3.utils.toWei("0"));
        assert.strictEqual((await plus.balanceOf(user1)).toString(), web3.utils.toWei("0"));
        assert.strictEqual((await plus.userShare(user3)).toString(), web3.utils.toWei("3"));
        assert.strictEqual((await plus.balanceOf(user3)).toString(), web3.utils.toWei("3.672"));
        assert.strictEqual((await plus.totalUnderlying()).toString(), web3.utils.toWei("3.672"));   // 0.024 fee remains!
        assert.strictEqual((await plus.liquidityRatio()).toString(), web3.utils.toWei("1"));
        assert.strictEqual((await token.balanceOf(user1)).toString(), "7128000");
        assert.strictEqual((await token.balanceOf(plus.address)).toString(), "0");
        assert.strictEqual((await token.balanceOf(strategy.address)).toString(), "3672000");
