...
contract("LiquidityGauge", async ([owner, governance, claimer, user1, user2, user3]) => {
    ...
    it("should set working balances", async () => {
        ...
        let iGauge = await IGauge.at(gauge.address);
        await iGauge.claim(user1, user2, true, {from: user1});
        ...
        await iGauge.claim(user1, user3, false, {from: user1});
    ...
    it("should distribute rewards in deposit and withdraw", async () => {
        ...
        let iGauge = await IGauge.at(gauge.address);
        await iGauge.claim(user1, user1, false, {from: user1});
        now = await time.latest();
        ...
        assertAlmostEqual((await gauge.claimable(user2)).toString(), toWei("3"));
        // 1.2 + 3 * 24 / 40 = 3
        ...
        await iGauge.claim(user2, user2, false, {from: user2});
    ...
    it("should update working balance in deposit and withdraw", async () => {
        let startTime = await time.latest();
        let now = await time.latest();
        await timeIncreaseTo(startTime);
        // NOTE: rate is in WAD!
        ...
        assertAlmostEqual((await gauge.claimable(user2)).toString(), toWei("2.325"));
        // 1.2 + 3 * 24 / 64 = 2.325
        assertAlmostEqual((await reward.balanceOf(user2)).toString(), toWei("0"));
        assertAlmostEqual((await controller.claimed(gauge.address, user2)).toString(), toWei("0"));

        await iGauge.claim(user2, user2, true, {from: user2});
    ...
    it("should update rewards on deposit and withdraw", async () => {
        let startTime = await time.latest();
        let now = await time.latest();
        await timeIncreaseTo(startTime);
        // NOTE: rate is in WAD!
        ...
        assert.strictEqual((await gauge.checkpointOf(user1)).toString(), "0");
        // Now user1 has 5 AC to claim
        assert.strictEqual((await gauge.rewards(user1)).toString(), "0");
        assertAlmostEqual((await gauge.claimable(user1)).toString(), toWei("5"));
        assert.strictEqual((await reward.balanceOf(user1)).toString(), "0");
        ...
        assertAlmostEqual((await gauge.integralOf(user2)).toString(), toWei("0.3125"));
        assert.strictEqual((await gauge.checkpointOf(user2)).toString(), now.toString());
        assert.strictEqual((await gauge.rewards(user2)).toString(), "0");
        assert.strictEqual((await gauge.claimable(user2)).toString(), "0");
        assert.strictEqual((await reward.balanceOf(user2)).toString(), "0");
        assert.strictEqual((await controller.claimed(gauge.address, user2)).toString(), "0");
        ...
        // user1 working supply = 40
        await voting.setTotalSupply(toWei("60"));
        await voting.setBalance(user1, toWei("50"));

        // user2 withdraws 35
        // working supply = 10 + 40 = 50
        // user1 working supply = 40
        // user2 working supply = 10
        await gauge.withdraw(toWei("35"), {from: user2});
        now = await time.latest();

        // 0.3125 + 2 / 40 = 0.3625
        assertAlmostEqual((await gauge.integral()).toString(), toWei("0.3625"));

        assertAlmostEqual((await gauge.integralOf(user1)).toString(), toWei("0"));
        assert.strictEqual((await gauge.checkpointOf(user1)).toString(), "0");
        assert.strictEqual((await gauge.rewards(user1)).toString(), "0");
        // 5 + 2 * 16 / 40 = 5.8
        assertAlmostEqual((await gauge.claimable(user1)).toString(), toWei("5.8"));
        assertAlmostEqual((await reward.balanceOf(user1)).toString(), "0");
        assertAlmostEqual((await controller.claimed(gauge.address, user1)).toString(), "0");

        assertAlmostEqual((await gauge.integralOf(user2)).toString(), toWei("0.3625"));
        // 2 * 24 / 40 = 1.2
        assertAlmostEqual((await gauge.rewards(user2)).toString(), toWei("1.2"));
        assertAlmostEqual((await gauge.claimable(user2)).toString(), toWei("1.2"));
        assert.strictEqual((await reward.balanceOf(user2)).toString(), "0");
        assert.strictEqual((await controller.claimed(gauge.address, user2)).toString(), "0");
        let iGauge = await IGauge.at(gauge.address);
        ...
        assert.strictEqual((await gauge.userStaked(user2)).toString(), toWei("0"));
        // 0.3625 + 3 / 50 = 0.4225
        assertAlmostEqual((await gauge.integral()).toString(), toWei("0.4225"));

        assertAlmostEqual((await gauge.integralOf(user1)).toString(), toWei("0.3625"));
        assert.strictEqual((await gauge.rewards(user1)).toString(), "0");
        // 3 * 40 / 50 = 2.4
        assertAlmostEqual((await gauge.claimable(user1)).toString(), toWei("2.4"));
        // 5 + 2 * 16 / 40 = 5.8
        assertAlmostEqual((await reward.balanceOf(user1)).toString(), toWei("5.8"));
        assertAlmostEqual((await controller.claimed(gauge.address, user1)).toString(), toWei("5.8"));

        assertAlmostEqual((await gauge.integralOf(user2)).toString(), toWei("0.4225"));
        assertAlmostEqual((await gauge.rewards(user2)).toString(), toWei("1.8"));
        assertAlmostEqual((await gauge.claimable(user2)).toString(), toWei("1.8"));
        // 1.2 + 3 * 10 / 50 = 1.8
        assertAlmostEqual((await reward.balanceOf(user2)).toString(), toWei("0"));
        assertAlmostEqual((await controller.claimed(gauge.address, user2)).toString(), toWei("0"));

        await iGauge.claim(user2, user3, true, {from: user2});
